...
function integrate_limit(f, f_zlimit, xmin, xmax)
    function hcfun(u)
        _x, _z = u
...
integrate_middle(f = bd.fi2m) = integrate_limit(f, (x) -> bd.zmax, bd.xmin0, bd.xmax0)
# dziob - psi2, fi2f
integrate_front(f = bd.fi2f) = integrate_limit(f, bd.psi2, bd.xmax0, bd.xmax)
# rufa - psi1, fi2r
integrate_rear(f = bd.fi2r) = integrate_limit(f, bd.psi1, bd.xmin, bd.xmin0)
